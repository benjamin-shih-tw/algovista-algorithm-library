...
import { ArrowLeft, BookOpen, ChevronLeft, ChevronRight, ExternalLink, Layers3, Pause, Play, RotateCcw, Search, Sparkles, Target } from 'lucide-react'
...
function GeometryScene({ lesson, frame }: { lesson: AlgorithmLesson; frame: Frame }) {
  ...
  if(lesson.id.includes('circle')) return <div className="scene geometry-scene circle-scene"><div className="scene-badge">CIRCLE GEOMETRY · {lesson.zhTitle}</div><svg viewBox="0 0 1000 430"><motion.circle cx="385" cy="220" r="130" className="geo-circle" initial={false} animate={{scale:1+(phase-1)*.038}} style={{transformOrigin:'385px 220px'}}/><motion.circle cx="625" cy="220" r="105" className="geo-circle secondary" initial={false} animate={{x:30-phase*15}}/><motion.line x1="385" y1="220" x2={phase===0?520:545} y2={phase===2?125:220} className="geo-construction"/><motion.line x1="625" y1="220" x2={phase===0?520:545} y2={phase===2?125:220} className="geo-construction"/><circle cx={phase===0?520:545} cy={phase===2?125:220} r="8" className="geo-focus"/><text x="385" y="225" className="geo-center">C₁</text><text x="625" y="225" className="geo-center">C₂</text></svg></div>
  if(/line|segment|dot|cross|polar|distance/.test(lesson.id)) return <div className="scene geometry-scene line-scene"><div className="scene-badge">VECTOR PREDICATES · {lesson.zhTitle}</div><svg viewBox="0 0 1000 430"><motion.line x1="120" y1="330" x2="850" y2="90" className="geo-main-line"/><motion.line x1="160" y1="85" x2="820" y2="350" className="geo-main-line secondary" animate={{opacity:phase===0 ? .3 : 1}}/><motion.path d="M 475 213 L 520 325 L 585 177" className="geo-angle" animate={{pathLength:(phase+1)/3}}/><circle cx="520" cy="325" r="8" className="geo-focus"/><text x="540" y="350" className="geo-label">orientation / projection</text></svg></div>
  if(/sweep|closest|voronoi|delaunay/.test(lesson.id)) return <div className="scene geometry-scene field-scene"><div className="scene-badge">SPATIAL STRUCTURE · {lesson.zhTitle}</div><svg viewBox="0 0 1000 430">{points.map((item)=><circle key={item.id} cx={item.x*10} cy={item.y*4.3} r="9" className="geo-point"/>)}{points.slice(0,-1).map((item,i)=><motion.line key={item.id} x1={item.x*10} y1={item.y*4.3} x2={points[(i+3)%points.length].x*10} y2={points[(i+3)%points.length].y*4.3} className="geo-mesh" animate={{opacity:i<=phase*3 ? .75 : .08}}/>)}<motion.line x1={220+phase*220} y1="20" x2={220+phase*220} y2="410" className="sweep-cursor"/></svg></div>
...
const framePhase = (frame: Frame) => Number(frame.state?.phase ?? Math.min(2, Math.max(0, frame.codeLines[0] % 3)))
...
function TraceOverlay({frame}:{frame:Frame}) {
  const trace=frame.trace!
  const activeIndex=trace.phase==='prepare'?0:trace.phase==='execute'?1:2
  return <motion.div className="trace-overlay" key={trace.signature} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:.28}}>
    <header><span>STEP-SPECIFIC TRACE</span><b>{String(trace.step+1).padStart(2,'0')} / {String(trace.totalSteps).padStart(2,'0')}</b></header>
    <div>{trace.nodes.map((node,index)=><motion.section key={`${node.label}-${index}`} className={index===activeIndex?'active':''} animate={{opacity:index<=activeIndex?1:.34}}><small>{node.label}</small><strong>{node.value}</strong>{index<trace.nodes.length-1&&<i>→</i>}</motion.section>)}</div>
  </motion.div>
}
...
function LinearScene({ lesson, frame }: { lesson: AlgorithmLesson; frame: Frame }) {
  ...
  if (isHeap) return <div className="scene structure-scene heap-scene"><div className="structure-title">BINARY HEAP · EXTREME AT ROOT</div><svg viewBox="0 0 1000 430">
    {[[500,80],[300,190],[700,190],[190,315],[410,315],[590,315],[810,315]].slice(1).map(([x,y],i)=>{const p=i<2?[500,80]:i<4?[300,190]:[700,190];return <motion.line key={i} x1={p[0]} y1={p[1]} x2={x} y2={y} className="structure-link" animate={{opacity:i<=phase*2+.5?1:.2}}/>})}
    {[[500,80],[300,190],[700,190],[190,315],[410,315],[590,315],[810,315]].map(([x,y],i)=><motion.g key={i} animate={{scale:i===Math.min(phase*2,6)?1.12:1}} style={{transformOrigin:`${x}px ${y}px`}}><rect x={x-45} y={y-28} width="90" height="56" rx="16" className="structure-node"/><text x={x} y={y+6} className="structure-value">{values[i%values.length]}</text></motion.g>)}
  </svg></div>
...
function TreeScene({ lesson, frame }: { lesson: AlgorithmLesson; frame: Frame }) {
  ...
  return <div className="scene tree-scene"><div className="scene-badge">ROOTED TREE · {lesson.zhTitle}</div><svg viewBox="0 0 1000 430">
    {treeLinks.map(([a,b],i)=><motion.path key={`${a}${b}`} d={`M ${node(a).x} ${node(a).y+28} C ${node(a).x} ${(node(a).y+node(b).y)/2}, ${node(b).x} ${(node(a).y+node(b).y)/2}, ${node(b).x} ${node(b).y-28}`} className={`tree-link ${i<=phase*2?'lit':''}`} animate={{pathLength:i<=phase*2?1:.25}}/>)}
    {treeLayout.map((item,i)=>{const active=frame.active?.includes(item.id)||i===phase*2;const accepted=frame.accepted?.includes(item.id)||phase===2&&i<5;return <motion.g key={item.id} animate={{y:active?-5:0,scale:active?1.08:1}} style={{transformOrigin:`${item.x}px ${item.y}px`}}><rect x={item.x-48} y={item.y-28} width="96" height="56" rx="17" className={`tree-node ${active?'active':''} ${accepted?'accepted':''}`}/><text x={item.x} y={item.y+5} className="tree-label">{item.id}</text><text x={item.x} y={item.y+46} className="tree-meta">{i===0?'depth 0':i<3?'depth 1':'depth 2'}</text></motion.g>})}
  </svg></div>
}
...
function RangeScene({ lesson, frame }: { lesson: AlgorithmLesson; frame: Frame }) {
  ...
  const phase=framePhase(frame), end=Math.min(values.length-1,2+phase*2)
...
function DPScene({ lesson, frame }: { lesson: AlgorithmLesson; frame: Frame }) {
  ...
  if (adaptive) return adaptive
  const phase=framePhase(frame), limit=[8,22,40][phase]??40
...
function StringScene({ lesson, frame }: { lesson: AlgorithmLesson; frame: Frame }) {
  ...
  if (adaptive) return adaptive
  const text=stringExample(lesson.id),phase=framePhase(frame),cursor=Math.min(text.length-1,phase*2+1)
...
function AlgorithmScene({lesson,frame}:{lesson:AlgorithmLesson;frame:Frame}) {
  ...
  return <div className="algorithm-scene-wrap" data-visual-model={lesson.visualModel}>{scene}<TraceOverlay frame={frame}/></div>
}
    ...
    <section className="lesson-stage">
      ...
      <div className="learning-workspace">
        <div className="visual-column">
          ...
          <div className="state-inspector"><span>ALGORITHM STATE</span><div>{Object.entries(frame.state ?? {}).map(([key,value])=><dl key={key}><dt>{key}</dt><dd>{Array.isArray(value)?value.join('  ·  '):value}</dd></dl>)}</div></div>
        </div>
        <aside className="code-panel"><header><div><i/><i/><i/><span>C++17</span></div><b>STEP-SYNCED SOURCE</b></header><pre>{lesson.code.map((line,lineIndex)=>{const active=frame.codeLines.includes(lineIndex+1);return <div key={lineIndex} className={active?'code-line active':'code-line'} aria-current={active?'step':undefined}><span>{String(lineIndex+1).padStart(2,'0')}</span><code><CppCode line={line}/></code></div>})}</pre></aside>
      </div>
      <div className="explanation-card"><span className="step-number">{String(index + 1).padStart(2,'0')}</span><AnimatePresence mode="wait"><motion.div key={`${lesson.id}-${index}`} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}}><span className="reasoning-label">CURRENT INVARIANT & ACTION</span><h2>{frame.title}</h2><p>{frame.explanation}</p><code><CppCode line={frame.codeLine}/></code></motion.div></AnimatePresence></div>
    </section>
    <footer className="player-controls"><div className="progress"><motion.i animate={{width:`${index/(lesson.frames.length-1)*100}%`}} /></div><span>{index+1} / {lesson.frames.length}</span><div><button aria-label="上一步" title="上一步" disabled={index===0} onClick={()=>{setPlaying(false);setIndex(index-1)}}><ChevronLeft/></button><button aria-label={playing?'暫停':'播放'} title={playing?'暫停':'播放'} className="play" onClick={()=>index===lesson.frames.length-1?restart():setPlaying(!playing)}>{playing?<Pause/>:<Play/>}</button><button aria-label="下一步" title="下一步" disabled={index===lesson.frames.length-1} onClick={()=>{setPlaying(false);setIndex(index+1)}}><ChevronRight/></button><button aria-label="重新播放" title="重新播放" onClick={restart}><RotateCcw/></button></div></footer>
  </main>
}

function LessonCard({lesson,onSelect}:{lesson:AlgorithmLesson;onSelect:(lesson:AlgorithmLesson)=>void}) { return <button className="lesson-card compact" onClick={()=>onSelect(lesson)} style={{'--lesson-accent':lesson.accent} as React.CSSProperties}><span className="card-index">{lesson.index}</span><span className="card-category">{lesson.category}</span><h2>{lesson.title}</h2><p>{lesson.zhTitle} · {lesson.description}</p><footer><span>{lesson.complexity}</span><b>開始學習 <ChevronRight size={15}/></b></footer></button> }
